...
3. Run `Flutter/package_windows_update.cmd`.
   - Confirm `NodeBackend/AppUpdates/ava-windows-<version>.zip` was created.
   ...
   - If another PC still cannot reach the backend, open TCP 8080 on the server
     PC from an admin prompt, for example with `New-NetFirewallRule -DisplayName "AVA NodeBackend 8080" -Direction Inbound -Protocol TCP -LocalPort 8080 -Action Allow`.
4. Build and copy the Android release APK for the same version.
   ...
   - Confirm `Flutter/build/app/outputs/flutter-apk/ava-android-<version>.apk`
     exists.
   - Confirm `NodeBackend/AppUpdates/ava-android-<version>.apk` exists.
...
5. Build and copy the macOS DMG for the same version when shipping macOS.
   ...
   - Confirm `NodeBackend/AppUpdates/AVA_Project_<version>_<build>_macOS.dmg`
...
6. Build and copy the iOS IPA for the same version when shipping iOS.
   - Confirm `NodeBackend/AppUpdates/ava-ios-<version>.ipa` exists, or set
