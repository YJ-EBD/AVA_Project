...
public class AvaAiService {
	...
	public List<AvaAiMessageResponse> history(AuthPrincipal principal) {
		...
		String companyName = normalizeCompanyName(profile == null ? null : profile.getCompanyName());
	...
	private AvaAiConversationEntity conversationFor(UserAccount account, String companyName, String firstMessage) {
		return conversationRepository.findByAccountId(account.getId())
	...
	private List<AvaAiLlmClient.PromptMessage> buildPrompt(
		...
		messages.add(new AvaAiLlmClient.PromptMessage(
			"system",
			systemPrompt(conversation.getCompanyName(), references, webResults)
		));
		...
		for (AvaAiMessageEntity message : recent.subList(fromIndex, recent.size())) {
			messages.add(new AvaAiLlmClient.PromptMessage(
				message.getRole() == AvaAiMessageRole.USER ? "user" : "assistant",
				message.getContent()
			));
		}
		if (recent.stream().noneMatch(message -> message.getId().equals(currentMessage.getId()))) {
			messages.add(new AvaAiLlmClient.PromptMessage("user", currentMessage.getContent()));
		}
		return messages;
	}
	...
	private String systemPrompt(
		...
		for (int index = 0; index < references.size(); index++) {
			...
			prompt.append("질문: ").append(limit(item.getQuestion(), 600)).append("\n");
			prompt.append("답변: ").append(limit(item.getAnswer(), 900)).append("\n");
		}
		return prompt.toString();
	}
	...
	private void appendWebSearchResults(StringBuilder prompt, List<WebSearchResult> webResults) {
		...
		for (int index = 0; index < webResults.size(); index++) {
				...
				prompt.append("Title: ").append(limit(result.title(), 180)).append("\n");
				prompt.append("URL: ").append(limit(result.url(), 300)).append("\n");
				prompt.append("Snippet: ").append(limit(result.snippet(), 500)).append("\n");
		}
	}
