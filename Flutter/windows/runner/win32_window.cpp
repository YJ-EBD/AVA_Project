...
constexpr int kCustomTitleBarHeight = 34;
...
int ScaleForWindow(HWND window, int source) {
  return MulDiv(source, GetDpiForWindow(window), 96);
}

LRESULT GetResizeHitTest(HWND window, LPARAM lparam) {
  POINT point = {GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam)};
  RECT rect;
  GetWindowRect(window, &rect);

  const int border = ScaleForWindow(window, kResizeBorderWidth);
  const int title_bar_height = ScaleForWindow(window, kCustomTitleBarHeight);
  const bool in_title_bar = point.y < rect.top + title_bar_height;

  // Keep the custom Flutter title bar fully interactive. Resizing is available
  // from the side and bottom edges below the title bar.
  if (in_title_bar) {
    return HTCLIENT;
  }
...
void RemoveNonClientFrame(HWND window, LPARAM lparam) {
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    case WM_NCCALCSIZE:
      if (wparam == TRUE) {
        RemoveNonClientFrame(hwnd, lparam);
        return 0;
      }
      break;

    case WM_NCHITTEST:
      return GetResizeHitTest(hwnd, lparam);

    case WM_GETMINMAXINFO: {
      auto info = reinterpret_cast<MINMAXINFO*>(lparam);
      info->ptMinTrackSize.x = ScaleForWindow(hwnd, 430);
      info->ptMinTrackSize.y = ScaleForWindow(hwnd, 560);
      return 0;
    }
